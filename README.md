...
对齐使用至少 4 秒的插值；较大移动按每秒 20 个校准归一化单位延长时间。
到达目标后最多等待 2 秒，六个关节都必须连续 5 次读回在 5 个归一化单位误差以内。
对承重导致的固定跟随偏差，稳定阶段会以每 0.1 秒最多 1 个单位、总计最多 8 个单位修正
leader 的命令目标；判定仍以实际读回角度为准。
这些单位不是角度。对齐期间 `Q/Esc` 或 `F` 可以中止并退出；提前按下的卸力/成功键不生效。
`events.jsonl` 中的 `leader_alignment_progress` 保存逐关节目标、实测位置和误差，失败信息也会
列出超差关节。对齐失败时主从臂均保持各自实测姿态，程序回到等待状态；操作员松手后可按
按键 1 重试。需要卸力手动调整时，先按 `Q/Esc` 结束本条。失败后不得跳过对齐直接进入人工接管。
