...
WATCH_LOG="$WS/logs/recap_v6_deploy_watcher_20260907.log"
SERVER_LOG="$WS/logs/recap_residual_v6_server_gpu1_20260907.log"

exec >>"$WATCH_LOG" 2>&1
echo "$(date -Is) waiting for v6 COMPLETE"
while [[ ! -f "$OUTPUT/COMPLETE" ]]; do
  sleep 10
done

echo "$(date -Is) validating v6 report"
...
echo "$(date -Is) deploying v6 on GPU1 port 8011"
...
for _ in $(seq 1 30); do
  if curl -fsS --max-time 2 http://127.0.0.1:8011/healthz; then
    echo
    echo "$(date -Is) v6 deployment ready"
    exit 0
  fi
  sleep 5
done
echo "$(date -Is) v6 server failed readiness timeout"
exit 1
