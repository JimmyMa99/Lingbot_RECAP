...
WATCH_LOG="$WS/logs/recap_v5_deploy_watcher_20260906.log"
SERVER_LOG="$WS/logs/recap_residual_v5_server_gpu1_20260906.log"

exec >>"$WATCH_LOG" 2>&1
echo "$(date -Is) waiting for v5 COMPLETE"
while [[ ! -f "$OUTPUT/COMPLETE" ]]; do
  sleep 10
done

echo "$(date -Is) validating v5 report"
...
echo "$(date -Is) deploying v5 on GPU1 port 8011"
...
for _ in $(seq 1 30); do
  if curl -fsS --max-time 2 http://127.0.0.1:8011/healthz; then
    echo
    echo "$(date -Is) v5 deployment ready"
    exit 0
  fi
  sleep 5
done
echo "$(date -Is) v5 server failed readiness timeout"
exit 1
