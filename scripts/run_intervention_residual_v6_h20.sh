...
PRE_TAKEOVER="$WS/experiments/lingbot_recap_straw_cup_pre_takeover_v5_20260906"
OUTPUT="$WS/experiments/lingbot_recap_straw_cup_intervention_residual_v6_20260907"
LOGS="$WS/logs"

export PYTHONPATH="$CODE:${PYTHONPATH:-}"
mkdir -p "$ENCODED" "$PRE_TAKEOVER" "$OUTPUT" "$LOGS"
...
run_distributed_stage() {
  ...
  for rank in 0 1 2; do
    CUDA_VISIBLE_DEVICES=$((rank + 1)) "$PY" "$CODE/tools/$tool" \
      ...
      >"$LOGS/${log_prefix}_rank${rank}_20260907.log" 2>&1 &
    pids+=("$!")
  done
  for pid in "${pids[@]}"; do
    wait "$pid"
...
run_distributed_stage encode_recap_experience.py "$ENCODED" recap_human_encode_v6
run_distributed_stage encode_recap_auto_context.py "$PRE_TAKEOVER" recap_pre_takeover_encode_v6 \
  --anchor-decisions 4

CUDA_VISIBLE_DEVICES=1 "$PY" "$CODE/tools/train_intervention_residual.py" \
  --encoded-root "$ENCODED" \
  --auto-root "$PRE_TAKEOVER" \
  --output-root "$OUTPUT" \
  --epochs 200 \
  --batch-size 128 \
  --lr 3e-4 \
  ...
  --held-out \
    ...
    episode_20260906_225709_43cf95ed.complete \
    episode_20260906_230510_3c6a187f.complete \
    episode_20260906_231149_8392fda6.complete \
    episode_20260906_231826_c82d96f9.complete \
    episode_20260907_003308_9c4660b6.complete \
    episode_20260907_004417_aa6e86bd.complete \
    episode_20260907_005358_8d22c01b.complete \
  2>&1 | tee "$LOGS/recap_intervention_residual_v6_20260907.log"
