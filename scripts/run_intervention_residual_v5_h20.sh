...
OUTPUT="$WS/experiments/lingbot_recap_straw_cup_intervention_residual_v5_20260906"
LOGS="$WS/logs"
...
run_distributed_stage encode_recap_experience.py "$ENCODED" recap_human_encode_v5
run_distributed_stage encode_recap_auto_context.py "$PRE_TAKEOVER" recap_pre_takeover_encode_v5 \
  --anchor-decisions 4
...
CUDA_VISIBLE_DEVICES=1 "$PY" "$CODE/tools/train_intervention_residual.py" \
  ...
  2>&1 | tee "$LOGS/recap_intervention_residual_v5_20260906.log"
